...
function SubjectRow({ subject, marks, onChange, error, isLast }) {
  ...
  return <div style={{ display: 'grid', gridTemplateColumns: '1fr 90px 90px 80px', alignItems: 'center', padding: '11px 24px', borderBottom: isLast ? 'none' : '1px solid #f8fafc', background: error ? '#fef2f2' : nonCredit ? '#fafafa' : 'white' }}>
    ...
    {nonCredit ? <><span /><span /><Input value={marks[subject.key]} onChange={value => onChange(subject.key, value)} id={`marks-${subject.key}`} max="100" /></> : <><Input value={marks[`${subject.key}_int`]} onChange={updateCie} id={`cie-${subject.key}`} /><Input value={marks[`${subject.key}_ext`]} onChange={updateSee} id={`see-${subject.key}`} /><div style={totalStyle}>{marks[subject.key] || 0}</div></>}
  </div>;
}

function Input({ value, onChange, id, max = '50' }) { return <div style={{ display: 'flex', justifyContent: 'center' }}><input id={id} type="number" min="0" max={max} placeholder={`0–${max}`} value={value ?? ''} onChange={event => onChange(event.target.value)} className="input-field" style={{ width: 72, textAlign: 'center', padding: '7px 8px', fontSize: 14 }} /></div>; }
